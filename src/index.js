...
import { sendLogin } from './utils/methods';
...
const init = (options = {}) => {
  ...
  if (!options.registerUrl) throw Error('请传入registerUrl参数');
  if (localStorage.getItem('userId')) {
    transitionOptions(_options, options);
  } else {
    sendLogin(options.registerUrl, device.deviceId).then((rs) => {
      localStorage.setItem('userId', rs);
      transitionOptions(_options, options, rs);
    });
  }
  // 将传过来的参数转换
  base.init(_options);
...
const transitionOptions = (_options, options, rs) => {
  const {
    requestUrl,
    appName,
    appCode,
  ...
  _options.userId = rs || localStorage.getItem('userId');
