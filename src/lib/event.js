...
function clickCollection() {
  document.addEventListener(
    ...
    (e) => {
      ...
      _config.pageName = localStorage.getItem('pageName')
...
function dwellCollector(eventUnload) {
  ...
  window.addEventListener(
    ...
    () => {
      ...
      _config.pageName = localStorage.getItem('pageName') //别骂，不得已而为之
      const mapping = {
 ...
 * @returns
 */
function traceCustomEvent(eventType, title, pageName, params = {}) {
  ...
  emit({
    ...
    referrer: document.referrer, //上一个页面的url
  })
}
